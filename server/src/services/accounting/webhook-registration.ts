...
export async function registerQboWebhook(db: Db, companyId: string): Promise<void> {
  const conn = await db
    .select({ realmId: accountingConnections.realmId })
    .from(accountingConnections)
    .where(and(eq(accountingConnections.companyId, companyId), eq(accountingConnections.platform, "quickbooks")))
    .limit(1)
    .then((rows) => rows[0] ?? null);

  if (!conn) {
    logger.warn({ companyId }, "registerQboWebhook: no QBO connection found");
    return;
  }

  logger.info(
    { companyId, realmId: conn.realmId },
...
export async function registerXeroWebhook(db: Db, companyId: string): Promise<void> {
  const conn = await db
    .select({ realmId: accountingConnections.realmId })
    .from(accountingConnections)
    .where(and(eq(accountingConnections.companyId, companyId), eq(accountingConnections.platform, "xero")))
    .limit(1)
    .then((rows) => rows[0] ?? null);

  if (!conn) {
    logger.warn({ companyId }, "registerXeroWebhook: no Xero connection found");
    return;
  }

  logger.info(
    { companyId, tenantId: conn.realmId },
