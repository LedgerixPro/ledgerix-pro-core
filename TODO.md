...
### Phase 4c.5: Admin API + Decision 4 + Decision 5 + Q1 + Q2 + Decision 6 + Decision 7
...
- [x] **P4c.5-14. Decision 6 + POST /api/accounting/v1/payments route** — LOCKED + FEATURE-COMPLETE Session 4 (2026-05-27). Second Phase 4c.5 write endpoint shipped end-to-end. Three Q-pay decisions locked: (Q-pay-1) `reconcilePayment` infers platform from accountingConnections — matches Decision 4/5 pattern; (Q-pay-2) service signature splits the overloaded `entityRef` into typed `customerId?` + `accountId?` while the payload preserves the single overloaded field per ADR-003 Q2 — the Piece F helper `resolveEntityRefByPlatform` is the single source of truth for translation, used by BOTH the route handler and the approval-replay path; (Q-pay-3) service returns `ReconcilePaymentResult` paralleling Decision 5's `UpdateTransactionCategoryResult`. Two implementation-time sub-decisions: (Q-pay-F-i) both helpers live in a single new file `payments-helpers.ts`; (Q-pay-F-ii) v1 ships without `expectedRange` in the payload — optional field stays in locked contract for future invoice-balance-comparison work. Pre-implementation Tenet #7 verification of 6 assumptions corrected an under-specified "2-3 hours" estimate into a properly-scoped 5-commit arc — pattern worth keeping: verify BEFORE estimating. Implementation arc: lock commit `0924fb94`, Piece D service refactor `37c55a08` (ReconcilePaymentResult + PaymentReferenceError + split-ref + platform inference), Piece F shared helpers `0d419021` (resolveEntityRefByPlatform + evaluatePaymentThreshold), Piece E approval-replay wiring `46f60b53` (PAYMENT_THRESHOLD_EXCEEDED stub replaced with real reconcilePayment), Piece G POST endpoint `41376751`. Test baseline 281 → 308 (+27 across the Decision 6 arc). End-to-end paths now operational: (1) direct programmatic via `reconcilePayment`; (2) HTTP endpoint with three response paths (200 success / 202 approval / 400 with two domain-specific codes `entity_ref_resolution_failed` + `payment_reference_invalid`); (3) approval-replay path. 2 of 4 executeApprovedAccountingWrite stubs now WIRED (transaction-category + payment-threshold); 2 of 4 await Invoice endpoint design.
- [x] **P4c.5-15. Decision 7 + POST /api/accounting/v1/invoices route** — LOCKED + FEATURE-COMPLETE Session 5 (2026-05-28). Third and final Phase 4c.5 write endpoint shipped end-to-end. Creates monthly service invoices in **Ledgerix Pro's own QBO** (not client books) behind two safety gates per the Trust Tenet. Three Q-inv sub-decisions locked at design time: (Q-inv-1) explicit `billingMode: "recurring" | "setup"` discriminator in the request body chooses the pricing function (recurring → `getExpectedPriceCents(tier, isCharter via isCharterForInvoicing(...))`; setup → `getSetupFeeCents(tier)`, no charter dimension per Q2); Q-inv-1-α — payload needs no `billingMode` field because an approved row is a human override and replay re-creates rather than re-validates. (Q-inv-2) Dedupe gate reuses `findOrCreateCustomer.action`; two ambiguous actions escalate to 202 with `accounting.invoice.dedupe_ambiguous` + fixed heuristic confidence (0.5 for `email_only_different_name`, 0.3 for `name_only` per Q-inv-2-α — honestly flagged as non-computed placeholders); Q-inv-2-β replay uses stored `matchedCustomerId`. (Q-inv-3) Pricing gate compares per-item-rounded line-item cents to expected cents at **zero tolerance** (Q-inv-3-α) because Ledgerix Pro owns both sides of the number; mismatch → 202 with `accounting.invoice.pricing_mismatch` + full audit fields; helper `evaluateInvoicePricing` lives in new `invoices-helpers.ts` (Q-inv-3-γ). **Q-inv-3-β REVISED mid-implementation** (commit `7ac02b90`, Tenet #16): `InvoicePricingMismatchPayload` carries no resolved `customerId`; locked Phase 4c.4 contract not amended; pricing_mismatch replay re-resolves via `findOrCreateCustomer` (Option A); ambiguous-on-replay drift escalates to `write_failed_replay` (conservative path); intentional asymmetry with `dedupe_ambiguous` replay documented. **Central design seam:** GHL contactId (used for pricing/charter/payload/audit) vs QBO books key (always `null` for own-QBO global connection — used in `findOrCreateCustomer` + `createInvoice` calls). **Money convention:** dollars in request body + lineItems (matches QBO wire format + Piece I replay), cents for all `*Cents`-suffixed pricing-decision/audit fields; conversion uses per-item rounding before summing — `lineItems.reduce((acc, li) => acc + Math.round(li.amount * 100), 0)`, NOT `Math.round(sum * 100)` — to avoid float-accumulation drift spuriously escalating valid multi-line invoices under the locked zero-tolerance comparison. Regression-locked with a divergent `[2.675, 2.675]` test (per-item 536¢ vs pre-round 535¢) that fails 201-vs-202 if the rounding order regresses. Implementation arc: design lock `a328db6f`, Q-inv-3-β REVISED `7ac02b90`, Piece H `287bb180` (`invoices-helpers.ts` — `evaluateInvoicePricing` + `confidenceForMatchType`; +8 tests), Piece I `441c8643` (approval-replay wiring for both invoice approval types — `executeApprovedAccountingWrite` reaches **4-of-4 wired**; -2 stubs + 9 new = +7 net), Piece K `9366445d` (POST /api/accounting/v1/invoices route; +17 tests). Piece J was a no-op — service-layer signatures already correct from Phase 4b. EA closeout `ac58de83` (§2B.4 REVISED note; 8-of-8 endpoints summary). PHASE-4-PROGRESS closeout `6fb3831c`. Test baseline 308 → 340 (+32 across the Decision 7 arc). **Phase 4c.5 endpoint roadmap: COMPLETE.** All 3 write endpoints shipped (category P4c.5-10 → payments P4c.5-14 → invoices this item). All 4 `executeApprovedAccountingWrite` approval-replay stubs wired (transaction-category + payment-threshold + invoice-dedupe + invoice-pricing). 8 of 8 Phase 4 endpoints production-ready. Q5 (multi-line journal write semantics, P4c.5-13) is the only remaining Phase 4c.5 decision and it gates no endpoint.
