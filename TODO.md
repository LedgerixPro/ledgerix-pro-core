...
### Phase 4c.5: Admin API + Decision 4 + Decision 5 + Q1 + Q2 + Decision 6
...
- [ ] **P4c.5-15. POST /api/accounting/v1/invoices re-implementation** — Architecturally unblocked from Q1 (Charter status) + Q2 (Setup fee handling) — both LOCKED + IMPLEMENTED Session 4 (2026-05-27). Design work remains: the endpoint must discriminate between recurring vs setup-fee billing modes in the request body, wire to `getExpectedPriceCents` + `isCharterForInvoicing` for recurring invoices vs `getSetupFeeCents` for setup invoices, and wire the two remaining `executeApprovedAccountingWrite` stubs (`accounting.invoice.dedupe_ambiguous` + `accounting.invoice.pricing_mismatch`) — three concerns in one endpoint vs the cleaner one-thing-per-endpoint pattern of P4c.5-10 (POST /category) and P4c.5-14 (POST /payments). Next-natural-step after Decision 6 closeout. Estimated ~4-6 hours including design-work session.
