...
### Phase 4c.5: Admin API + Decision 4 + Decision 5
...
- [ ] **P4c.5-11. Q1: Charter status storage** — Architectural decision pending. `getExpectedPriceCents` requires `isCharter` parameter; no defined storage exists yet. ADR-003 Amendment 1 Gap 1. Blocks Invoice endpoint re-implementation. Entangled with business-model considerations.
- [ ] **P4c.5-12. Q2: Setup fee handling** — Architectural decision pending. Setup fees ($249/$349/$1,200 per pricing spec Section 7) not modeled by current pricing schema. ADR-003 Amendment 1 Gap 2. Blocks Invoice endpoint re-implementation.
