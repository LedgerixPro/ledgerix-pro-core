...
- `2a44cfdf` — Decision L (L1-revised) locked in WIP doc.
- `91772d7d` — Step 3: mounted `requireAgentPermission(db, key)` as inline per-route middleware on the category (`accounting:write_category`) and payments (`accounting:create_payment`) write routes (Decisions I1/J1). Invoices route intentionally ungated (I1). Updated 2 pre-existing agent write-route tests to mock `hasPermission→true` (L1-revised). Added K1 integration tests on both gated routes: ungranted-agent→403 with the **403-before-400 ordering assertion PASSING** (sent body `{}` that would otherwise 400; got 403 — proves the gate is the outermost gate per Decision G, with service mock `.not.toHaveBeenCalled()` and `hasPermission` called with `("agent", key)`); granted-agent reaches handler; non-agent pass-through. accounting.test.ts 92→96 tests, all green; server typecheck clean.
...
3. ✅ DONE (91772d7d): ✅ DECISIONS LOCKED (I1/J1/K1): Mount `requireAgentPermission(db, key)` as inline per-route middleware on the category + payments routes (NOT invoices — Decision I1). Add integration tests per K1 including the 403-before-400 ordering assertion that proves the Decision G outermost-gate placement. — L1-revised: mount real middleware + vi.mock access service; UPDATE the two pre-existing agent write-route tests (accounting.test.ts:1820 category, :2139 payments) to mock hasPermission→true; ADD K1 tests (ungranted-agent 403-before-400, granted-agent reaches handler, non-agent pass-through).
4. ✅ DONE (90ba922a + 91772d7d): Tests: middleware unit tests (agent-with-grant 200-path, agent-without-grant 403, non-agent pass-through) + route integration tests. Unit tests landed with step 2 in `require-agent-permission.test.ts` (6 tests covering all branches incl. defensive missing-identity). Route integration tests landed with step 3 in accounting.test.ts (4 new K1 tests across both gated routes + 2 updated pre-existing agent tests). The agent allowlist enforcement path is covered by the K1 integration tests on both gated routes.
...
- Shipped step 3 (91772d7d): agent allowlist mounted on category+payments write routes; invoices intentionally ungated (I1). The Decision G outermost-gate placement is PROVEN — the 403-before-400 ordering test passes on both routes (ungranted agent gets 403 even when body would 400, service never invoked). 92→96 tests green. The two pre-existing agent write-route tests updated to mock-grant per L1-revised. Phase 5 enforcement is functionally complete; step 5 closeout (ADR migration) is next.
