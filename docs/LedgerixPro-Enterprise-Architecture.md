...
| `GET /transactions/:txnId` | Production-ready (Decision 4 FEATURE-COMPLETE) | Unified `getTransactionById` dispatcher covering all 11 planned transaction types via 10 handler functions (7 QBO + 3 Xero; Xero Invoice + Bill share `fetchXeroInvoiceOrBill` per the REVISED note — Xero serves ACCREC and ACCPAY from the same endpoint with a Type discriminator). Phase 1 + Phase 2 foundation + full Phase 2 type expansion all shipped Session 4 (2026-05-27, commits `bffa3b16` through `4e9d70be`). 37 new tests across the implementation arc (baseline 161 → 198). Returns `previousAccountRef` for audit trails on write-endpoint re-implementations. |
...
| `POST /transactions/:txnId/category` | Production-ready (Phase 4c.5 SHIPPED end-to-end) | Thin wrapper around `updateTransactionCategory` (Decision 5 dispatcher, services/accounting/transaction-write.ts). Three response paths per ADR-003 Q4: (1) success → 200 with `{ platform, txnType, txnId, previousAccountRef, newAccountRef }` audit trail; (2) `TransactionNotFoundError` → 202 with approval row creation (type `accounting.transaction.category_with_unknown_previous`, wired to executeApprovedAccountingWrite); (3) `TransactionTypeNotCategorizableError` → 400 for the 5 excluded Decision 5 types (QBO BillPayment/Payment/Invoice, QBO JournalEntry, Xero ManualJournal). Full ADR-003 Q5 idempotency support via existing `withIdempotency` wrapper. Properly separates `requestedByUserId` (board actors) vs `requestedByAgentId` (agent actors) on approval rows — FK safety fix caught pre-commit during implementation. Shipped Session 4 (2026-05-27) commit `bfc8549d` (Piece C); Decision 5 + Pieces A/B/C arc spans commits `07c056e5` through `bfc8549d` (10 commits, +35 tests, baseline 198 → 233). |
| `POST /api/accounting/v1/payments` | Production-ready (Phase 4c.5 SHIPPED end-to-end) | Decision 6 dispatcher (`reconcilePayment`, services/accounting/index.ts) refactored for split-ref signature + audit-trail return + platform inference per Q-pay-1/2/3. Three response paths per ADR-003 Q4: (1) success → 200 with `ReconcilePaymentResult` audit trail (`{ platform, paymentId, invoiceId, amount, customerId?, accountId?, paymentDate }`); (2) threshold exceeded → 202 with approval row creation (type `accounting.payment.threshold_exceeded`, wired to executeApprovedAccountingWrite via the Piece F resolver — same translation logic used by both route and replay path); (3) two domain-specific 400 codes — `entity_ref_resolution_failed` (connection-state issue) + `payment_reference_invalid` (resolved ref doesn't match resolved platform). Full ADR-003 Q5 idempotency support via `withIdempotency` wrapper. FK-safe actor separation matching Piece C pattern. Sub-decision Q-pay-F-ii honored — payload omits the optional `expectedRange` field (deferred from v1 to avoid doubling upstream latency per payment; field stays in locked contract for future invoice-balance comparison). Shipped Session 4 (2026-05-27) commit `41376751` (Piece G); Decision 6 arc spans commits `0924fb94` through `41376751` (5 commits, +27 tests, baseline 281 → 308). |
| `POST /api/accounting/v1/invoices` | Production-ready (Phase 4c.5 SHIPPED end-to-end) | Decision 7 invoice-creation endpoint. Creates monthly service invoices in **Ledgerix Pro's own QBO** (not client books) behind two safety gates per the Trust Tenet: (1) **dedupe gate** — calls `qbo.findOrCreateCustomer`; the two ambiguous actions (`ambiguous_name_only` / `ambiguous_email_match_different_name`) escalate to 202 with approval row creation (type `accounting.invoice.dedupe_ambiguous`, fixed heuristic confidence values 0.3 / 0.5 per Q-inv-2-α); (2) **pricing gate** — `billingMode: "recurring"` → expected = `getExpectedPriceCents(tier, isCharter via isCharterForInvoicing(...))`; `billingMode: "setup"` → expected = `getSetupFeeCents(tier)`. Zero-tolerance comparison (Q-inv-3-α) of per-item-rounded line-item cents vs expected cents; any non-zero delta escalates to 202 with approval (type `accounting.invoice.pricing_mismatch`, full `pricingDecision` audit fields). Both gates passed → 201 Created with `qbo.createInvoice` (QBO books key always `null` per the own-QBO global connection seam; GHL contactId used for pricing/charter/payload/audit context only). Two domain-specific 500 codes — `pricing_not_configured` + `setup_fee_not_configured` (operational seeding gaps, NOT caller-correctable 400s). Full ADR-003 Q5 idempotency via `withIdempotency` wrapper. FK-safe actor separation matching Piece C/G pattern. Money convention: dollars in body/lineItems (matches QBO wire format), cents for all `*Cents`-suffixed audit fields; per-item rounding before sum (load-bearing for the zero-tolerance comparison — regression-locked via a divergent `[2.675, 2.675]` test where pre-round summing diverges by 1¢). Shipped Session 5 (2026-05-28) commit `9366445d` (Piece K); Decision 7 arc spans commits `a328db6f` (design lock) through `9366445d` — Pieces H (`287bb180`, invoices-helpers), I (`441c8643`, approval-replay wiring → dispatcher 4-of-4), K (`9366445d`, route); Piece J was a no-op (`findOrCreateCustomer` / `createInvoice` already had correct signatures from Phase 4b). Mid-implementation Q-inv-3-β REVISED (`7ac02b90`, Tenet #16): pricing_mismatch replay re-resolves customer via `findOrCreateCustomer` (payload carries no resolved customerId; locked Phase 4c.4 contract not amended; ambiguous-on-replay drift escalates to `write_failed_replay`). +32 tests (baseline 308 → 340). |

8 of 8 endpoints production-ready (POST /invoices via Decision 7 shipped end-to-end Session 5 2026-05-28, joining POST /transactions/:txnId/category via Decision 5 and POST /payments via Decision 6). The Phase 4c.5 write-endpoint roadmap is complete; all 4 `executeApprovedAccountingWrite` approval-replay stubs are wired (transaction-category, payment-threshold, invoice-dedupe, invoice-pricing).
...
- **~~Q1: Charter status storage~~** — LOCKED + IMPLEMENTED 2026-05-27 Session 4 as Option B (local DB table). `getExpectedPriceCents` requires `isCharter` parameter; source of truth is now table `client_charter_status` with status enum `active` / `cancelled_was_charter` / `never_charter`. Trust Tenet #14 invoked — billing source of truth in our own DB (not GHL), with the cancellation-forfeits-Charter rule structurally enforced via the enum (one-way transition from `active` → `cancelled_was_charter`; `never_charter` → `active` forbidden). Service module `server/src/services/accounting/charter.ts` exports `getCharterStatus`, `isCharterForInvoicing` (the Invoice endpoint's primary call), and 3 mutation helpers (`grantCharterToNewClient`, `recordNonCharterClient`, `cancelCharter`) with typed errors for forbidden transitions. Shipped: lock commit `0cf679d6`, implementation commit `5b4856bb`, migration `0068_youthful_blockbuster.sql`. Test baseline 233 → 253 (+20). See `docs/wip/phase-4c-5-write-endpoints-and-admin-api.md` Q1 for the full locked contract + deferred-work tracking (onboarding/cancellation workflow integration not yet wired).
- **~~Q2: Setup fee handling~~** — LOCKED + IMPLEMENTED 2026-05-27 Session 4 as Option B (parallel table). `service_tier_pricing` only models monthly recurring; setup fees now live in a separate `setup_fee_pricing` table with NO isCharter column (per EA Section 7: setup fees don't vary by Charter status). Aligns with Q1's design principle — each business concern gets its own table. Service function `getSetupFeeCents(db, tier): Promise<{ amountCents, priceRecordId }>` added to `server/src/services/accounting/pricing.ts` alongside the existing `getExpectedPriceCents`. Seed values from EA Section 7: Foundation $249 / Growth Engine $349 / Scale-Up $1,200. Admin endpoint `POST /api/admin/pricing/seed` extended to seed both tables in a single call with combined response shape `{ data: { pricing: {...}, setupFees: {...} }, meta }` (sub-decision Q2-α-i locked during implementation). Shipped: lock commit `0cf679d6`, implementation commit `83b80a72`, migration `0069_damp_bloodscream.sql`. Test baseline 253 → 262 (+9). See `docs/wip/phase-4c-5-write-endpoints-and-admin-api.md` Q2 for the full locked contract + sub-decision Q2-α-i documentation + deferred-work tracking (Invoice endpoint wiring + production seed invocation not yet done).
- **Q5: Multi-line journal write semantics (QBO JournalEntry + Xero ManualJournal category updates)** — PENDING. Surfaced during Decision 5 scoping (Session 4, 2026-05-27). Both types were explicitly excluded from Decision 5 because updating one line's AccountRef without offsetting changes breaks journal balance. The architectural problem requires its own decision: how does the caller express intent — preserve balance? update both Debit and Credit lines simultaneously? require dual-AccountRef in the request? — and what business workflows need this capability (rare in practice based on Decision 4 read-side traffic patterns). Until Q5 is decided, calls to `updateTransactionCategory` for JournalEntry/ManualJournal txns throw `TransactionTypeNotCategorizableError` (correctly routed by the dispatcher's exclusion list). See `docs/wip/phase-4c-5-write-endpoints-and-admin-api.md` Decision 5 → "Out of scope for Decision 5 (deferred)" subsection.
- ~~**Q3: get-transaction-by-id infrastructure scope**~~ — RESOLVED Session 3 (2026-05-26) as Decision 4 (Option A — full coverage), **FEATURE-COMPLETE Session 4 (2026-05-27)**. Per-type fetch handlers for all 11 planned transaction types now live behind the unified `getTransactionById` interface returning `previousAccountRef` to callers. **Implementation arc**: Phase 1 (commit `bffa3b16` — dispatcher + 3 types), Phase 2 foundation (commit `635e4998` — `HttpResponseError` class + strict dispatcher discriminator), and Phase 2 type expansion across 6 commits: `8830f206` JournalEntry, `7027c79a` Deposit, `2195544a` BillPayment, `769a39ca` Payment, `bf96d2d3` Invoice (QBO half complete), `4e9d70be` Xero Invoice/Bill/ManualJournal (feature-complete). **Decision 4 REVISED note** (commit `fb13f98c`, per Tenet #16) documents that Xero serves ACCREC and ACCPAY from the same `/Invoices/{id}` endpoint, requiring a shared `fetchXeroInvoiceOrBill` handler under two registry keys — final implementation: 11 type keys / 10 handler functions. Test baseline 161 → 198 (+37 tests). See `docs/wip/phase-4c-5-write-endpoints-and-admin-api.md` Decision 4 for the full implementation arc.
- **Decision 5: Write-side dispatcher scope (transaction category updates)** — LOCKED + FEATURE-COMPLETE Session 4 (2026-05-27). The write-side counterpart to Decision 4. Covers 6 of 11 read-supported transaction types via 5 handler functions (Xero Invoice + Bill share `updateXeroInvoiceOrBillAccount` per the Decision 4 REVISED shared-handler pattern). 5 EXCLUDED types throw `TransactionTypeNotCategorizableError`: QBO BillPayment / Payment / Invoice (top-level or Item-based account refs, not "category" semantics) + QBO JournalEntry / Xero ManualJournal (multi-line Debit/Credit balance preservation deferred to Q5). Per Tenet #14 (Trust Tenet, "no partial-spec compliance on safety-critical write endpoints") the asymmetry with Decision 4 is by design — matches QBO/Xero API constraints. Implementation arc: locked commit `07c056e5`, foundation `69505e90`, 5 per-type handlers `d90d5304`/`034ac5c4`/`eb77d817`/`5f30c3b2`/`e7ee3273`, final integration `b7da7478` (legacy method deletion + hintedType extension), approval-replay wiring `001d547f` (per ADR-003 Q2), POST endpoint `bfc8549d`. Test baseline 198 → 233 (+35 across Decision 5 + Pieces A/B/C). See `docs/wip/phase-4c-5-write-endpoints-and-admin-api.md` Decision 5 for the full locked spec + implementation arc.
- **Decision 6: POST /payments scope (Q-pay-1/2/3 + Q-pay-F-i + Q-pay-F-ii)** — LOCKED + FEATURE-COMPLETE Session 4 (2026-05-27). Second Phase 4c.5 write endpoint after Decision 5. Three primary sub-decisions locked at the architectural layer: (Q-pay-1) `reconcilePayment` infers platform from accountingConnections — matches Decision 4/5 pattern, callers drop platform param; (Q-pay-2) service signature splits the overloaded `entityRef` into typed `customerId?` + `accountId?` while the payload preserves the single overloaded field per ADR-003 Q2 — the Piece F helper `resolveEntityRefByPlatform` is the single source of truth for translation, used by BOTH the route handler and the approval-replay path; (Q-pay-3) service returns `ReconcilePaymentResult` paralleling Decision 5's `UpdateTransactionCategoryResult` — captures platform-assigned paymentId previously discarded by void returns. Two implementation-time sub-decisions: (Q-pay-F-i) both helpers (`resolveEntityRefByPlatform` + `evaluatePaymentThreshold`) live in a single new file `payments-helpers.ts`; (Q-pay-F-ii) v1 ships without `expectedRange` in the `PaymentThresholdExceededPayload` — the optional field stays in the locked contract for future invoice-balance-comparison work but is not populated by v1. Q-pay-4 (threshold check location) was settled by the existing payload shape — route layer creates approvals, so threshold check happens there; Q-pay-5 (approval-replay path) follows from Q-pay-1. Implementation arc: lock commit `0924fb94`, service refactor `37c55a08` (Piece D — ReconcilePaymentResult + PaymentReferenceError + split-ref + platform inference), shared helpers `0d419021` (Piece F — resolveEntityRefByPlatform + evaluatePaymentThreshold), approval-replay wiring `46f60b53` (Piece E — PAYMENT_THRESHOLD_EXCEEDED stub replaced with real reconcilePayment), POST endpoint `41376751` (Piece G — production route). Test baseline 281 → 308 (+27 across the Decision 6 arc). Pre-implementation Tenet #7 verification surfaced 6 honest findings about the pre-existing infrastructure (zero callers on legacy `reconcilePayment`, no threshold integration, no idempotency, void return deviating from 17+ in-file conventions, overloaded entityRef parameter, established Decision 4/5 platform-inference pattern) — the verified findings informed Decision 6's lock, correcting an under-specified "2-3 hours" estimate into a properly-scoped 5-commit arc. Pattern worth keeping: verify BEFORE estimating. See `docs/wip/phase-4c-5-write-endpoints-and-admin-api.md` Decision 6 for the full locked spec + implementation arc.
- **Decision 7: POST /invoices scope (Q-inv-1/2/3 + Q-inv-3-β REVISED)** — LOCKED + FEATURE-COMPLETE Session 5 (2026-05-28). Third and final Phase 4c.5 write endpoint; creates monthly service invoices in **Ledgerix Pro's own QBO** (not client books) behind two safety gates per the Trust Tenet. Three sub-decisions locked at design time: (Q-inv-1) explicit `billingMode: "recurring" | "setup"` discriminator in the request body chooses the pricing function (recurring → `getExpectedPriceCents(tier, isCharter)`; setup → `getSetupFeeCents(tier)`, no charter dimension per Q2); the locked Phase 4c.4 approval payloads need no `billingMode` field because an approved row is a human override and replay re-creates rather than re-validates (Q-inv-1-α). (Q-inv-2) Dedupe gate reuses `findOrCreateCustomer.action` rather than reimplementing dedupe; the route reads `action` and gates on the two ambiguous values with fixed heuristic confidence (0.5 for `email_only_different_name`, 0.3 for `name_only`; Q-inv-2-α, honestly flagged as non-computed placeholders since `findOrCreateCustomer` doesn't compute a numeric score); dedupe_ambiguous replay uses the stored `matchedCustomerId` (Q-inv-2-β). (Q-inv-3) Pricing gate compares per-item-rounded line-item cents to expected cents at zero tolerance (Q-inv-3-α) because Ledgerix Pro owns both sides of the number — no rounding/FX slack warranted; helper `evaluateInvoicePricing` lives in new `invoices-helpers.ts` (Q-inv-3-γ). **Q-inv-3-β REVISED mid-implementation (commit `7ac02b90`, Tenet #16):** the `InvoicePricingMismatchPayload` carries `customerName` + `customerEmail` but NOT a resolved `customerId` (unlike `InvoiceDedupeAmbiguousPayload` which does); locked Phase 4c.4 contract not amended; pricing_mismatch replay re-resolves customer via `findOrCreateCustomer` (Option A); ambiguous-on-replay drift escalates to `write_failed_replay` (conservative path — the human's pricing approval does not authorize resolving a fresh dedupe ambiguity); intentional asymmetry with dedupe_ambiguous replay documented. **Central design seam — identifier semantics:** the GHL contactId identifies *which client to bill* (used for pricing/charter/payload/audit) while the QBO books-connection key is always `null` for Ledgerix Pro's own-QBO global connection (used in `findOrCreateCustomer` + `createInvoice` calls). **Money convention:** dollars in request body / lineItems (matches QBO wire format and Piece I replay), cents for all `*Cents`-suffixed pricing-decision/audit fields; conversion uses per-item rounding before summing (load-bearing for the locked zero-tolerance comparison — regression-locked via a divergent `[2.675, 2.675]` test where pre-round summing diverges by 1¢). Implementation arc: design lock `a328db6f`, mid-implementation Q-inv-3-β REVISED `7ac02b90`, Piece H `287bb180` (`invoices-helpers.ts` — `evaluateInvoicePricing` + `confidenceForMatchType`), Piece I `441c8643` (approval-replay wiring for both invoice approval types — `executeApprovedAccountingWrite` reaches 4-of-4 wired), Piece K `9366445d` (POST /api/accounting/v1/invoices production route). Piece J was a no-op (`findOrCreateCustomer` / `createInvoice` already had correct signatures from Phase 4b). Test baseline 308 → 340 (+32 across the Decision 7 arc). **REVISED 2026-05-28 (§2B.4 supersession, Tenet #16):** The Phase 4 API Spec §2B.4 (v1.0, May 16) specified that POST /invoices performs "no automatic validation" of pricing and that "the billing agent is responsible." Decision 7 supersedes this. The shipped endpoint enforces pricing validation at the route (zero-tolerance comparison of line-item total against `getExpectedPriceCents` / `getSetupFeeCents`) and escalates both pricing mismatches and ambiguous customer dedupe to human approval (202 + `accounting.invoice.pricing_mismatch` / `accounting.invoice.dedupe_ambiguous`). Agent-trust is replaced by endpoint-enforced safety per the Trust Tenet. The API Spec §2B.4 language is stale and should be read through this note. See `docs/wip/phase-4c-5-write-endpoints-and-admin-api.md` Decision 7 for the full locked spec + implementation arc.
