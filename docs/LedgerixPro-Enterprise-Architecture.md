...
| `GET /transactions/:txnId` | Partial (Decision 4 Phase 1 + Phase 2 foundation shipped, type expansion pending) | Needed by write endpoints for `previousAccountRef` capture. Decision 4 Phase 1 (Session 3, commit `bffa3b16`) ships the unified `getTransactionById` dispatcher with 3 of 11 types: QBO Purchase, QBO Bill, Xero BankTransaction. Phase 2 foundation (Session 4, commit `635e4998`) adds structured `HttpResponseError` class to qboRequest/xeroRequest and tightens dispatcher's multi-type probing to strict discriminator (only 404 continues; everything else rethrows). Remaining type expansion: 5 QBO + 3 Xero types (~3-4 hours). |
