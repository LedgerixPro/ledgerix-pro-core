...
| `POST /api/accounting/v1/invoices` | DEFERRED (Phase 4c.5) | Architecturally unblocked from Q1 (Charter status) + Q2 (Setup fee handling) — both LOCKED + IMPLEMENTED Session 4 (2026-05-27). Design work remains: the endpoint must discriminate between recurring vs setup-fee billing modes in the request body, wire to `getExpectedPriceCents` + `isCharterForInvoicing` for recurring invoices vs `getSetupFeeCents` for setup invoices, and wire the two remaining `executeApprovedAccountingWrite` stubs (`accounting.invoice.dedupe_ambiguous` + `accounting.invoice.pricing_mismatch`) — three concerns in one endpoint vs the cleaner one-thing-per-endpoint pattern of POST /category and POST /payments. Next-natural-step after Decision 6's Session 4 close. |

6 of 8 endpoints production-ready (POST /transactions/:txnId/category via Decision 5 + POST /payments via Decision 6, both shipped end-to-end Session 4 2026-05-27). Only POST /invoices remains deferred — design work pending after Decision 6 closeout.
