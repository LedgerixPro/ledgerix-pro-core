...
- **Q-charter — onboarding/cancellation workflow wiring (architectural arc, not a wiring task).** Tenet #7 verify on 2026-05-29 reframed this from "code work" to "multi-session architectural arc." Five findings: (1) Onboarding agent appears in `server/src/services/dispatcher.ts` as a named dispatch target for `contact.created` / `contact.updated` events, but the agent's actual implementation (prompt file + logic) was not confirmed by verify — same pattern as the Billing & Invoicing finding. (2) No GHL event in the dispatcher map represents "client became a paying client" — `opportunity.won` is the closest semantic match but it routes to Client Success Manager, not Onboarding; the became-paying trigger is architecturally undefined. (3) EA Section 8's Charter eligibility rule (first 10 paying clients + Charter follows the client across tier changes) requires atomic count-of-active-Charter-clients enforcement at grant time — the charter service does not currently expose such a function. Service-layer gap, not wiring gap. (4) No client-cancellation handler exists in code. `cancelCharter` has nowhere to be called from today; wiring requires CREATING the cancellation handler from scratch (which itself requires deciding the trigger — Stripe webhook? GHL pipeline stage? Admin endpoint?). (5) Same situation for `recordNonCharterClient`. **Resolution path:** when the next session opens this arc, frame it as a real lock-then-implement decision with at minimum these sub-decisions: which event(s) signal "became-paying," how to atomically enforce the first-10 cap, what triggers the cancellation handler. Until then, the three Charter service functions remain callable-but-uncalled and `isCharterForInvoicing` continues to default to `false` for any client without an explicit `client_charter_status` row (safe billing default).
